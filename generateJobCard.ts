...
const repeatingHeader = ({ doc, booking, serviceCenterAddress }: any) => {
  ...
  doc
    .font(FONT_BOLD)
    .fontSize(9)
    .text(booking?.jobcard_number, 27, y + 43, {
      width: boxWidth,
      align: "right",
    });

  const date = moment(booking?.booking_date).format("DD MMM YYYY");
  ...
  doc.image(
    path.join(__dirname, "../public/LocationIcon.png"),
    boxWidth + 17,
    y + 65,
    { width: 10 },
  );
...
export const jobCardPdf = async (jobCardData: any) => {
  const {
    booking,
    SpareTotal,
    ServiceTotal,
  ...
  doc
    .font(FONT_BOLD)
    .text(`Spare Total: ₹${SpareTotal}`, boxWidth - 120, tableEndY + 10);

  doc
    .font(FONT_BOLD)
    .text(`Labor Total: ₹${ServiceTotal}`, boxWidth - 40, tableEndY + 10);
  ...
  doc
    .fontSize(8)
    .font(FONT_BOLD)
    .text(`Total: ₹ ${totalAmount}`, 20, TotalBoxY + padding, {
      width: boxWidth,
      align: "right",
    });
  ...
  accessoriesOthers.forEach((item: any, index: any) => {
  ...
  doc
    .font(FONT_BOLD)
    .fillColor("#333333")
    .text("CUSTOMER SIGNATURE", 20, termsY + 40);
